...
#| label: load-packages
#| echo: FALSE
# this code chunk loads the tidyverse package
...
```{r}
#| label: create-vectors-add-letters-punch
...
```{r}
#| label: created-symbols-dataframe
# clean, rename, and assign numbers for symbols dataframe

my_symbols <- c(lower_case, punctuation, upper_case)

symbols <- data.frame("symbol" = my_symbols)

total_symbols <- length(symbols$symbol)

number <- seq(1, total_symbols, 1)

symbols$assign_number <- number
...
```{r}
#| label: add-mutlitply-exponentiate-square
# arithmetic for the top_secret values
